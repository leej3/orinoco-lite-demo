...
description: Run a downstream's configured Orinoco Lite metadata adapters against site-owned source configuration and evidence with explicit human decisions. Use when capturing source snapshots, generating candidate changes, reviewing accept/reject/defer choices, updating compact durable decision state, applying decisions, opening adapter pull requests, checking provenance, or determining whether local locks or newer upstream behavior governs. Do not use for ordinary editorial or asset work.
---
...
1. Read every applicable `AGENTS.md`, the site README, ownership manifest, `orinoco.lock` or equivalent release evidence, the applicable `site-specific/sources/<id>/source.yaml`, adapter README, tests, and site-owned policy or decision registers.
2. Confirm that the checkout is a released downstream and resolve the adapter from its source configuration.
Reusable adapters are bundled in `orinoco-lite`; site-specific adapters live under `extensions/source-adapters/<adapter>/`.
The thin template supplies the workflow caller and source configuration without copying generic adapter executables.
...
2. Capture or select the declared source snapshot through the configured workflow or the adapter's documented command.
The shared host runs as `python -m orinoco_lite.source_adapters.review --root . review` and obtains its schema and helpers from the package.
Keep literal project-relative arguments in the execution record and run focused tests for any changed site-specific adapter before a network-backed refresh.
