...
DECISION_CACHES = {
    adapter: PurePosixPath("site-specific", "curation-records", f"{adapter}.yaml")
    for adapter in ADAPTERS
}
PROVIDERS = {
    "dump-research-info": PurePosixPath(
        "extensions/source-adapters/dump-research-info/candidates.py"
    ),
    "zotero": PurePosixPath(
        ".orinoco-lite/source-adapters/zotero/candidates.py"
    ),
}
...
def _relative_record_path(value: object) -> str:
    ...
    if (
        ...
        or path.parts[: len(RECORD_ROOT.parts)] != RECORD_ROOT.parts
        or len(path.parts) <= len(RECORD_ROOT.parts)
        or any(part in {"", ".", ".."} or part.startswith(".") for part in path.parts)
    ):
        raise CurationHostError("Record path is outside site-specific/metadata/records")
    return rendered
...
def _safe_metadata_path(value: object) -> str:
    ...
    if (
        ...
        or (
            ...
            and path.parts[: len(ANNOTATION_ROOT.parts)] != ANNOTATION_ROOT.parts
        )
    ):
    ...
    for entry in history:
        ...
        for parent in parents:
            for status, rendered in _diff_entries(root, parent, commit):
                ...
                if (
                    path.parts[: len(RECORD_ROOT.parts)] != RECORD_ROOT.parts
                    and path.parts[: len(ANNOTATION_ROOT.parts)]
                    != ANNOTATION_ROOT.parts
                ):
                    raise CurationHostError(
...
def verify_proposal_commit(
    ...
    actual_records = {
        ...
        if PurePosixPath(path).parts[: len(RECORD_ROOT.parts)] == RECORD_ROOT.parts
    }
    expected_records = {
...
def _load_provider(trusted_root: Path, adapter: str) -> ModuleType:
    ...
    path = trusted_root.joinpath(*PROVIDERS[adapter].parts)
...
@contextmanager
def _candidate_path_environment(root: Path):
    """Bind Candidate defaults to this downstream's configured record root."""

    workspace = load_workspace(root)
    configured = {
        "ORINOCO_ROOT": os.fspath(workspace.root.resolve()),
        "ORINOCO_RECORDS_ROOT": os.fspath(workspace.path("records").resolve()),
    }
    previous = {name: os.environ.get(name) for name in configured}
...
def build_plan(
    ...
    *,
    ...
    if adapter == "dump-research-info":
        ...
        with _candidate_path_environment(root), redirect_stdout(sys.stderr):
