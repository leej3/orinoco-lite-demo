#!/usr/bin/env python3
"""Run configured downstream metadata source adapters."""
...
CONFIG = ROOT / "site-specific" / "sources"
...
def load_config(path: Path = CONFIG) -> list[dict[str, object]]:
    if path.is_dir():
        documents = sorted(path.glob("*/source.yaml"))
        if not documents:
            raise MetadataReviewError(
                f"Adapter source root contains no source.yaml files: {path}"
            )
    elif path.is_file():
        documents = [path]
    else:
        raise MetadataReviewError(f"Adapter source configuration is absent: {path}")
    seen: set[str] = set()
    result: list[dict[str, object]] = []
    for document_path in documents:
        try:
            source = yaml.safe_load(document_path.read_text(encoding="utf-8"))
        except (OSError, UnicodeDecodeError, yaml.YAMLError) as error:
            raise MetadataReviewError(
                f"Cannot read adapter source configuration {document_path}: {error}"
            ) from error
        if not isinstance(source, dict):
            raise MetadataReviewError(
                f"Adapter source configuration must be a mapping: {document_path}"
            )
        if source.get("contract_version") != ADAPTER_API_VERSION:
            raise MetadataReviewError(
                f"Adapter source contract_version is unsupported: {document_path}"
            )
        source_id = source.get("id")
        if not isinstance(source_id, str) or not source_id or source_id in seen:
            raise MetadataReviewError(f"Invalid or duplicate source id {source_id!r}")
        if (
            document_path.name == "source.yaml"
            and document_path.parent.parent == path
            and document_path.parent.name != source_id
        ):
            raise MetadataReviewError(
                f"Adapter source id does not match its directory: {document_path}"
            )
...
def allowed_destination(root: Path, source_id: str, value: object) -> Path:
    ...
    source = root / "site-specific" / "sources" / source_id
    allowed = tuple((source / name).resolve() for name in ("content", "evidence"))
    if not any(destination == prefix or destination.is_relative_to(prefix) for prefix in allowed):
        raise MetadataReviewError(
            f"Evidence destination is outside the source content/evidence roots: {destination}"
        )
