...
- the reviewed adapter provenance identity from trusted
  `site-specific/sources/dump-research-info/source.yaml`, with a matching
  `xyzri:XYZInstrument` record already present in the record tree.
...
The adapter's optional compact cache is
`site-specific/curation-records/dump-research-info.yaml`; a missing file is the
valid empty state.
