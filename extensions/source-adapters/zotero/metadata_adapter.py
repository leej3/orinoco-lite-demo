...
def load_tools() -> tuple[ModuleType, ModuleType]:
...
def review(context: Mapping[str, object]) -> dict[str, object]:
    from orinoco_lite.source_adapters.review import semantic_diff
    ...
    canonical_publications = yaml_map(
        load_workspace(root).path("records") / "XYZPublication"
    )
