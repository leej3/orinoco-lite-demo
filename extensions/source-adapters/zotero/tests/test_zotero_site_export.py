...
SCRIPTS = ROOT / "extensions/source-adapters/zotero"
    ...
    def test_export_rejects_destinations_outside_repository_build_state(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "strict descendant"):
                self.export(root / "outside", build_root=build)
            with self.assertRaisesRegex(ValueError, "strict descendant"):
                self.export(
                    build, output=build, report=build / "report.json", build_root=build,
                )
            with self.assertRaisesRegex(ValueError, "dedicated build artifacts"):
                self.export(build, output=build / "another-build-artifact")
