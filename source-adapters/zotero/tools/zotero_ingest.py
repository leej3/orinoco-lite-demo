...
This script intentionally does not promote records into ``metadata/records``
...
def source_items(snapshot: dict[str, Any]) -> list[SourceItem]:
...
def classify(item: SourceItem) -> tuple[str | None, str | None, str | None]:
    ...
    if item_type == "document":
        ...
        if "zenodo/osf dois" in collection_set:
...
def command_transform(args: argparse.Namespace) -> None:
    ...
    group_id = int(snapshot.get("source", {}).get("group_id") or DEFAULT_GROUP_ID)
    ...
    for group_key in sorted(groups):
        ...
        classifications = [(item, *classify(item)) for item in selected_group]
        class_names = {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    fetch.add_argument("--group-id", type=int, default=DEFAULT_GROUP_ID)
    ...
    fetch.add_argument(
        "--user-agent",
        default="orinoco-lite/0.1 (CON public Zotero ingestion)",
    )
